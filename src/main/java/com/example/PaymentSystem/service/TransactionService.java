...
public class TransactionService {
        ...
        public TransactionResponse transfer(TransferRequest request, UUID initiatedByUserId) {
                ...
                if (targetUser == null) {
                        targetUser = userRepository.findByEmail(recipientIdStr)
                                        .orElseThrow(() -> new ResourceNotFoundException(
                                                        "Recipient user not found for: " + recipientIdStr));
                }
                ...
                // Fallback: if no wallet is explicitly marked primary, use the oldest active
                // wallet
                if (target == null) {
                ...
                try {
                        ...
                        // PostgreSQL translates each lock to: SELECT * FROM wallets WHERE id = ? FOR
                        // UPDATE
                        // ---------------------------------------------------------------
        ...
        public TransactionResponse refund(RefundRequest request, UUID initiatedByUserId) {
                ...
                walletRepository.findByIdWithLock(refundFirstLockId)
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "Wallet not found during refund lock"));
